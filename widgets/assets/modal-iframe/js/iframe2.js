    ...
    function initEvents() {
        ...
        $(pub).on('open.iframe.gromver', function(e, data, source) {
            var action = data.action,
                ...
                iframeOptions = data.iframeOptions;
            ...
            if (formOptions) {
                ...
                if (window.location.pathname == action) {
                    //баг с отображением тойже страницы что отображена в родительском окне, пусто, можно добавить в какойнить мусор
        ...
        $(pub).on('send.iframe.gromver', function(e, data, source) {
            //parentRelation(source).yii.gromverIframe.dataHandler(data);
